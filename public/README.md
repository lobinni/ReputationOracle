...
**A reusable GenLayer Intelligent Contract primitive that turns publicly available web sources into a verified, on-chain stream of reputation assessments.**

This is a **standalone Intelligent Contract** submission — not a project, not a dApp, not a frontend application. The repository contains only the contract, its tests, documentation, examples, and development tooling.
...
You cannot hash a web page to detect that. Session identifiers, timestamps, layout changes, and A/B copy mutate on every request, so a byte-diff fires constantly and tells you nothing.
...
A counterparty publishes audit reports and compliance pages. A protocol escrows funds against their reputation. Both need an answer to _"did their credibility materially change?"_ Neither can be the one who answers it.
...
| --- | --- |
| **Off-chain watcher + signed feed** | The operator decides what "reputation" means. Every party must trust them. |
| **Chainlink or any price oracle** | There is no numeric quantity to report. "Audit failed" is not a feed value. |
| **Content hash on-chain** | Timestamps, session IDs alter the hash every request. |
| **Deterministic HTML parser** | Breaks on reorder. A vendor can hide changes by rewording. |
| **A single LLM call off-chain** | Someone must be trusted to run it honestly. |
...
| --- | --- |
| _"An AI app with GenLayer attached"_ | The output is not advice. It is a typed state transition — score, confidence, signal diff — consumed programmatically by other contracts. In `examples/defi_risk_gate.py` it unlocks withdrawal. |
| _"A validator that only checks output format"_ | Neither equivalence principle checks shape. Round 1 requires validators to agree that two signal sets carry the same **information**; round 2 requires score and confidence to **match exactly**. Valid JSON with a different verdict fails consensus. |
| _"Judging facts from user-submitted text"_ | No fact about the world is ever accepted from a caller. The only inputs are entity name and URL (immutable after creation — there is no `set_url`). Every recorded signal was extracted by the contract, from its own stored URL, inside a consensus block. |
| _"A thin LLM wrapper"_ | The model is one step. Around it sit anchored canonicalization, a deterministic digest gate, a scoring ladder, monotonic owner constraints, failure semantics that never mutate state, and a subscriber fan-out. Remove the consensus and the contract has no reason to exist; remove the surrounding machinery and the consensus is unusable. |
...
| `gl.nondet.exec_prompt` (extraction) | round 1 | Reducing prose to canonical signals is a language-understanding task. A deterministic parser can extract a `<div>`; it cannot recognise that "regulatory compliant" and "meets all requirements" are the same signal, which is the entire point. |
...
* **The change decision itself.** `digest(signals) == stored_digest` is plain Keccak256 over a sorted signal list, computed outside every consensus block. When sources have not moved, "nothing changed" is a _deterministic_ answer that no model participates in.
...
### Round 1 — Signal Extraction

The page is fetched inside the consensus block and reduced to an ordered set of `key → value → polarity` signals. The previous signal set is fed back as an anchor (both keys and values) so an unchanged page reproduces an identical snapshot.
...
Confidence levels (1-3) indicate how much evidence supports the assessment:

| Confidence | Meaning |
| --- | --- |
| 1 LOW | Few sources, speculative, or conflicting information. |
| 2 MEDIUM | Reasonable evidence from multiple sources. |
| 3 HIGH | Strong, corroborated evidence across sources. |

Only changes that affect the score bump the version, append to history, and notify subscribers.

---

## Monotonic Owner Constraints — Suppression Resistance

The whole point of consensus is that no single party authors the answer. That breaks if the profile owner can, after the fact, mute the report. These constraints exist to prevent exactly that.

| Property | Constraint | Why |
| --- | --- | --- |
| entity_name | **No setter.** | Renaming would invalidate every subscriber's assumption about what is being assessed. |
| criteria | **No setter.** | The criteria define what "reputation" means. Changing it would alter all future scores. |
| url | **No setter.** | Repointing would let an entity swap in favorable pages. |
| set_min_alert_score | **May only be raised.** | Lowering would suppress alerts subscribers expect. Owners wanting less sensitivity create a new profile. |
...
if not state["reliable"]:
    ...   # paused or degraded: do not assume stability
```
...
| **One deployment, many profiles, many subscribers** | Shared infrastructure. Consumers do not deploy their own copy; they call `create_profile` or `subscribe` on an existing one. |
| **An event source** | Deliberately the most composable output shape available. A push callback plus a pull-readable version means both reactive and polling consumers work. |
| **Safe-by-default trust model** | A consumer does not have to audit the profile owner. The owner's powers are constrained _by the contract_ — `min_alert_score` can only be raised, `cooldown` can only be lowered, `url` has no setter, and the subscriber picks its own alert floor. |
...
| **Typed interface** | `IReputationSubscriber` and `IReputationOracle` are importable stubs; integration is autocompleted and type-checked. |
...
| Method | Description |
| --- | --- |
| `create_profile(entity_name, url, criteria, min_alert_score=3, cooldown_seconds=3600)` → u256 | Register entity and take baseline assessment. Costs one observation round. |
...
| Method | Description |
| --- | --- |
| `subscribe(profile_id, alert_below=3)` | Register the caller for callbacks at a threshold **they** choose. One entry per address. |
...
| JSON Recovery | plain JSON, fenced JSON, JSON with prose, already-parsed dicts, garbage rejection |
...
| **Suppression Resistance** | `min_alert_score` cannot be lowered, `cooldown` cannot be raised, new owner inherits constraints, `entity_name`/`criteria`/`url` have no setters |
...
✅ Lint clean  
✅ **27+ direct tests pass** covering all pure functions, edge cases, and adversarial scenarios  
